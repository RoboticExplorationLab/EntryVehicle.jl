...
function dyna_coeffoff_COM_on_axis(t, x, u)
    ...
    m = m_p + m_cone #total mass in kg
    mu = 42828.37 #km^3.s-2
    r_G = [0.0;0.0;0.3] #COM in body frame of global spacecraft
    δ = 70*pi/180 #radians
    r_cone = 1.3 #in meters
    h_cone = r_cone/tan(δ)
    Re = 3389.5 #Radius in km
    ...
    Cτ = table_Cτ[α, :]
    h = (norm(r)-1.0)*Re
    F_aero_body = -0.5*exponential_atmosphere(h)*((norm(v_rel)*1e3)^2)*CF
    τ_aero_body = -0.5*exponential_atmosphere(h)*((norm(v_rel)*1e3)^2)*Cτ
    F_aero_eci = qrot(q, F_aero_body)
