...
function dyna_coeffoff_inplace_sphere!(du, u, p, t) #For DiffEq

    #@show(t)
    m = 0.1 #kg
    mu = 42828.37 #km^3.s-2
    r_sphere = 0.05 #meters
    Re = 3389.5 #Radius in km
    ...
    #CF = table_CF[α, :]
    #Cτ = table_Cτ[α, :]
    A_sphere = pi*r_sphere^2
    h = (norm(r)-1.0)*Re
    F_aero_body = -0.5*exponential_atmosphere(h)*norm(v_rel)^2*(v_rel/(norm(v_rel)))*A_sphere
    τ_aero_body = [0.0; 0.0; 0.0] #-0.5*exponential_atmosphere(h)*norm(v_rel)^2*Cτ
    F_aero_eci = qrot(q, F_aero_body)
