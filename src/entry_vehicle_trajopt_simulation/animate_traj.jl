...
for i = 1:N
    MeshCat.atframe(anim,vis,i) do frame
        settransform!(frame["vehicle"], compose(Translation(Z[i][1:3].*10...),LinearMap(Quat(qmult(Z[i][4:7], QQ)...))))
    end
    setobject!(vis["traj"]["t$i"],sphere_small, green_material)
    settransform!(vis["traj"]["t$i"], Translation(Z[i][1]*10, Z[i][2]*10, Z[i][3]*10))
