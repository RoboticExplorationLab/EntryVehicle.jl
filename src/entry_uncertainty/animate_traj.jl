...
for i = 1:N
    MeshCat.atframe(anim,vis,i) do frame
        T = Z[14, i]
        #material = MeshPhongMaterial(color = C[floor(Int, 1+(T-300.0)/9)-1])
        #setobject!(vis["vehicle"]["t$i"], cap, material)
        settransform!(frame["vehicle"], compose(Translation(Z[1:3, i].*10...),LinearMap(Quat(qmult(Z[4:7, i], QQ)...))))
        settransform!(vis["/Cameras/default"], Translation(Z[1:3, i].*10...))
    end
