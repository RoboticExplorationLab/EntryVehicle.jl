...
function dyna_coeffoff(t, x, u, w)
    ẋ = zeros(13)
    ...
    #Compute aerodnyamic forces
    ω_mars = [0; 0; 7.095*10^(-5)]
    v_rel = (v-cross(ω_mars, r*Re)) + w[2:end] #velocity of spacecraft wrt atm
    ...
    Cτ = table_Cτ[α, :]
    h = (norm(r)-1.0)*Re
    F_aero_body = -0.5*exponential_atmosphere(w[1], h)*norm(v_rel)^2*CF
    τ_aero_body = -0.5*exponential_atmosphere(w[1], h)*norm(v_rel)^2*Cτ
    F_aero_eci = qrot(q, F_aero_body)
