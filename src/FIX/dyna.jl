...
function dyna_coeffoff_COM_on_axis_lincov(t, x, u)
    #ẋ = zeros(13)
    ẋ = zeros(eltype(x), length(x))
    ...
    G = mat_euler_rate_quat_rate(e)
    q̇ = 0.5*qmult(q, [0; ω])
    W = mat_euler_quat_rate_mat(e)
    V = W*G

    ẋ[1:3] = v
    ẋ[4:6] = 0.5*inv(V)*ω
    ẋ[7:9] = (F_total_eci/m)
    ẋ[10:12] = Jinv*(τ_total_body-cross(ω, J*ω))
else
    ẋ = zeros(12)
end
    return ẋ
