...
function marsgram_wrapper(name; kwargs...)
    options = kwargs
    write_file(name; options...)
    execute_marsgram(name)
    sleep(1.0) #seems to be necessary otherwise, no time to create the file.
    data_table = output_data_extraction(options[1])
    return data_table
end

X = marsgram_wrapper("test6"; OUTFL = "OUT18", NPOS = 200)
